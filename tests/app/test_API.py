...
#from app import MicroserviceAPI
#
#@pytest.fixture
#def client():
...
def test_get_one_products_no_products(client):
    ...
    response = client.get('/products/api/v1.0/products/get_product/', json = request_payload)

    assert response.status_code == 404

def test_get_all_offers_no_offers(client):
    response = client.get('/products/api/v1.0/products/get_all_offers')

    assert response.status_code == 404

def get_offers_for_one_product_no_products(client):
    request_payload = {"id": 1}
    response = client.get("/products/api/v1.0/products/get_offers_one_products", json = request_payload)
...
def test_add_product(client):
    ...
    response = client.post("/products/api/v1.0/products/add_product", json=request_payload)
...
def test_update_product(client):
    ...
    request_payload = {"description": "test description"}
    response = client.put("/products/api/v1.0/products/update_product", json=request_payload)
    result = response.get_json()
...
def test_get_offers_one_product(client):
    ...
    # No id in request
    request_payload = {"test": "test"}
    response = client.get("/products/api/v1.0/products/get_offers_one_product/", json=request_payload)
...
def test_delete_product(client):
    ...
    # No id in request
    request_payload = {"test": "test"}
    response= client.delete("/products/api/v1.0/products/delete_product/", json=request_payload)
